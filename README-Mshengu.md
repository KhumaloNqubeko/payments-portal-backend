...
Create a local `.env` file from `.env.example`, then set values for your
PostgreSQL database, JWT secret, and SSL keystore password:
...
The project is configured for SonarQube analysis through Maven, with JaCoCo
coverage generated during `verify`.

To start a local SonarQube server:
...
account, change the password when prompted, and create a project token.

Then run:

```bash
mvn -B "-Dspring.profiles.active=test" verify sonar:sonar -Dsonar.host.url=http://localhost:9000 -Dsonar.token=YOUR_SONAR_TOKEN
```

For GitHub Actions, configure these repository secrets:

- `SONAR_HOST_URL`, for example `http://your-sonarqube-host:9000`
- `SONAR_TOKEN`, created from your SonarQube user or project
